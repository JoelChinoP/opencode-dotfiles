...
If the project has no Playwright test dependency, stop and ask the user whether
to add one with the project's package manager and version policy. The managed
`playwright-cli` browser tool is not authorization to change project
dependencies; never bootstrap `@latest` automatically.
